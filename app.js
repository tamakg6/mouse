...
function startTargetTask() {
    ...
    targetMoveHandler = (e) => {
        ...
        targetData.push({ task:'target', event:'move', x, y, t, tid:targetsDone, tx:'', ty:'' });
...
function renderTargetTrail() {
    ...
    for (let i = 1; i < trailPts.length; i++) {
        const alpha = (i / trailPts.length) * 0.2;
        tCtx.beginPath();
...
function spawnTarget() {
    ...
    const area   = document.getElementById('target-area');
    const MARGIN = 72;
    const maxX   = area.offsetWidth  - TARGET_SIZE - MARGIN;
    const maxY   = area.offsetHeight - TARGET_SIZE - MARGIN;

    const left = MARGIN + Math.random() * maxX;
    const top  = MARGIN + Math.random() * maxY;
    const cx   = left + TARGET_SIZE / 2;
    const cy   = top  + TARGET_SIZE / 2;

    const div = document.createElement('div');
    div.className = 'target-circle';
    // width・height を必ず px で指定 → border-radius:50% で丸になる
    div.style.width  = TARGET_SIZE + 'px';
    div.style.height = TARGET_SIZE + 'px';
...
function startFreeTask() {
    ...
    fMoveHandler = (e) => {
        ...
        freeData.push({ task:'free', event:'move', x, y, t, tid:'', tx:'', ty:'' });
...
function renderFreeTrail() {
    ...
    for (let i = 1; i < fTrail.length; i++) {
        const alpha = (i / fTrail.length) * 0.25;
        fCtx.beginPath();
